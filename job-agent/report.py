...
def generate_report_html(
    ...
    state = state or {}
    applied_count = sum(1 for job in state.values() if job.get("status") in ("applied", "interviewing"))
    ...
    applied_section = ""
    if applied_matches:
        applied_section = _section("Applied", applied_matches, "")
    interested_section = ""
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .card.interviewing {{
    border-color: #7a3d9e;
    border-width: 2px;
    background: color-mix(in srgb, #7a3d9e 5%, var(--surface));
  }}
...
def write_report(
    new_matches, interested_matches, applied_matches, scored_count, fetched_count,
    state=None, output_path=None, generated_at=None,
):
    if output_path is None:
        output_path = os.path.join(os.path.dirname(__file__), "report.html")
    html_content = generate_report_html(
        new_matches, interested_matches, applied_matches, scored_count, fetched_count, state, generated_at
    )
