...
manual_extras.json with any cover letter text / salary estimate figures
for this run, then run this script. It goes through the same
pipeline.process_run() as match_jobs.py, so dedup, seen-job tracking, and
report generation behave identically regardless of scoring source.
"""
...
new_matches, tracked_matches, state = process_run(data["matches"], cover_letter_fn, salary_estimate_fn)
